...
pub mod pallet {
    ...
    impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
        fn build(&self) {
            ...
            for (acct, dna, gender) in &self.kitties {
                _ = <Pallet<T>>::mint(acct, Some(dna.clone()), Some(gender.clone()));
            }
        }
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn buy_kitty(
            ...
            ensure!(T::Currency::free_balance(&buyer) >= (bid_price + deposit), <Error<T>>::NotEnoughBalance);
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn mint(
            ...
            ensure!(Self::kitties(&kitty_id) == None, <Error<T>>::KittyAlreadyExists);
